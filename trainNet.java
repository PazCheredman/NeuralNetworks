...
public class trainNet {
	...
	static network net1Hidden = new network();
	...
	public static void main(String[] args) {
		...
		net1Hidden.setRo(learningRate);
		Layers lay = new Layers(10000, 1000);
		net1Hidden.add(lay);
		lay = new Layers(lay, 125);
		net1Hidden.add(lay);
		...
		for (int i = 0; i < 20; i++) {
			...
			runSerial(19, net1Hidden);
			int[] ansSerial= checkError(20);
		...
		System.out.println("The training is on 10 learning groups");
		for (int i = 0; i < 5; i++) {
			System.out.println();
			System.out.println("This is iteration number: "+i);
			runNonSerial(5,net1Hidden);
			int[] ansNonSerial= checkError(6);
	...
	public static void processFile(String dirK, int i, int j, int k){
		...
		net1Hidden.forward(input/*, output*/);
		double expected[]= new double[3];
		expected[0]= 1;
		expected[1]= 0;
		expected[2]= 0;
		net1Hidden.back(expected);
	...
	public static int[] checkError(int k){
		...
		for (int i = 0; i < 20; i++) {
			...
			net1Hidden.forward(input/*, output*/);
			output = net1Hidden.getLast().getOutputArray();
